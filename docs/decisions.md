...
### 007 — Disponibilidad multi-AZ y escalabilidad: red ya preparada, cómputo fuera de alcance por ahora

**Decision:** la VPC de `app-01` tiene dos subredes en dos AZ distintas (`pixelhub-subnet-app` en AZ A, `pixelhub-subnet-app-b` en AZ B), ambas asociadas a la misma route table y por lo tanto con el mismo acceso al VPC Gateway Endpoint de S3. `app-01` en sí sigue siendo una única instancia EC2 — no se agrega Auto Scaling Group ni Load Balancer en este componente.

**Contexto:** en la revisión del proyecto se pidió tener en cuenta disponibilidad multi-AZ y escalabilidad. El alcance elegido (ADR 001) es explícitamente un componente — storage — no la migración completa del cómputo, así que había que decidir hasta dónde llevar esa recomendación sin desviarse del alcance.

**Alternativas:**
(a) Dejar todo en una sola AZ (como estaba) y posponer multi-AZ por completo hasta que se migre el cómputo.
(b) Ir directo a Auto Scaling Group + Application Load Balancer multi-AZ para `app-01` ahora mismo.
(c) (elegida) Dejar la red ya preparada para multi-AZ — dos subredes, dos AZ, mismo acceso a S3 — sin tocar el cómputo todavía.

**Tradeoff:** la opción (b) resolvería disponibilidad y escalabilidad de punta a punta, pero suma 2 servicios más (ASG, ELB) y complejidad fuera del alcance que se decidió cubrir en profundidad (ADR 001) — hubiera diluido el foco del proyecto. La opción (a) dejaba la recomendación del profesor sin ninguna respuesta concreta. La opción (c) es el punto medio: no resuelve la disponibilidad de `app-01` todavía (sigue siendo un SPOF, ver tabla de SPOFs en `architecture.md`), pero elimina el rediseño de red que haría falta el día que se agregue el ASG — y del lado del storage, S3 ya es multi-AZ por diseño (11 nueves de durabilidad, replicado entre AZ), así que esa mitad del problema ya estaba resuelta desde el arranque del proyecto.

**Resultado:** `scripts/vpc_demo.py` crea `pixelhub-subnet-app` (AZ A) y `pixelhub-subnet-app-b` (AZ B), ambas en la misma route table que tiene la ruta al Gateway Endpoint de S3. Próximo paso natural (fuera de este componente): Auto Scaling Group de `app-01` distribuido en ambas subredes + Application Load Balancer.
